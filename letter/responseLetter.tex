...
{\color{blue}{\bf Answer.} We restructured Section 3.3 to detail our approach better. We created sub Section 3.2.1 to present data extraction process of our First study, and 3.2.2 to present data analysis procedure. In some way, we created sub Section 3.3.1 and 3.3.2, to address the same issues, referring to our second study. With this restructuring we better clarify our study setting Section, presenting more details about our first (Section 3.2) and second study (Section 3.3). We also created a picture that can help to understand the methodology of our second study (Figure 3). }  
...
{\color{blue}{\bf Answer.} Thanks for this comment. We make available our tool, datasets and
...
{\color{blue}{\bf Answer.} We agree. To mitigate this issue, we include at Section 6 (Conclusion and future work Section) one last paragraph that points how our results can be useful for researches at Android sandboxes. In the same paragraph, we also highlight which next steps can contribute to research evolution.}
...
{\color{blue}{\bf Answer.} We improved the abstract, making it clearer. We better contextualized ``mining sandbox approach'', presenting its definition e importance for Android malware analysis.}
...
testing~\cite{jamrozikZ16}. But in 3.3, you are comparing the source-sink pairs between that of a benign app and its malign twin. It is already 
...
{\color{blue}{\bf Answer.} You are right: we actually use a curate set of sensitive sources and sinks
  ...
  We changed the first paragraph of Section 3.3.1 to clarify this point. Thanks.
...
{\color{blue}{\bf Answer.} Our approach in the second study aims to find divergences in the source-sink
  ...
  {\bf are not considered suspicious}. Only the paths in the second execution, which do not appear in the first execution, are considered suspect and are used to point out the malware. We changed Section 3.3.2 to
...
{\color{blue}{\bf Answer.} We respectfully disagree with the ``mere replication''
  opinion. Indeed, the relevance of replication in software engineer
...
{\color{blue}{\bf Answer.} We explained better the sentence at Introduction, paragraph 2.}
