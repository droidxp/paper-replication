...
behavior by identifying the divergent source-sink paths that FlowDroid finds in the
...
We conduct this investigation in three steps. First, we define
the set of API calls considered as \emph{source of sensitive information} (e.g.,
a method that access the address book) or
that \emph{might share information} (e.g., a method that sends messages
to external peers). We rely on a the source-sink definitions of a previous
study~\cite{}. After defining the sources and sinks of interest,
we run FlowDroid to mine the source-sink paths from each benign app, and then enumerate the
possible data flows between sources and sinks. These paths are considered valid in
our analysis. Next, we repeat the the FlowDroid execution considering the malicious version
of the apps. In the final step, we compare the sets of source-sink paths between the benign and malicious
version of an app, in order to reveal the existence of additional paths in the
malicious version.

\begin{figure}
\includegraphics[scale=0.3]{images/second-study-settings.pdf}
\end{figure}
